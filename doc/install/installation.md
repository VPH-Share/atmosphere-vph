...
VhpShare Atmosphere installation consists of setting up the following components:
...
5. Upstart scripts
6. VphShare Atmosphere application
7. Nginx web server
8. VphShare Atmosphere administrator account
9. Logrotate
...
template1=# CREATE DATABASE vphshare_production OWNER atmosphere;

# Quit the database session
template1=# \q

# Try connecting to the new database with the new user
sudo -u atmosphere -H psql -d vphshare_production
```
...
Add the following to `/home/atmosphere/.bash_profile`:
...
sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/doc/install/support/git/post-receive -O /home/atmosphere/current/.git/hooks/post-receive
...
# Download templates
sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/doc/install//support/upstart/master.conf.erb -O /home/atmosphere/upstart-templates/master.conf.erb

sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/doc/install//support/upstart/process.conf.erb -O /home/atmosphere/upstart-templates/process.conf.erb

sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/doc/install//support/upstart/process_master.conf.erb -O /home/atmosphere/upstart-templates/process_master.conf.erb
...
sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/config/air.yml.example -O /home/atmosphere/current/config/air.yml

sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/config/puma.rb.example -O /home/atmosphere/current/config/puma.rb

sudo -u atmosphere -H wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/config/initializers/action_mailer.rb.example -O /home/atmosphere/current/config/initializers/action_mailer.rb
...
sudo wget --no-check-certificate https://raw.githubusercontent.com/VPH-Share/atmosphere-vph/master/doc/install/support/nginx/atmosphere-vph -O /etc/nginx/sites-available/atmosphere-vph

# customize nginx configuration file
sudo editor /etc/nginx/sites-available/atmosphere-vph

# ...enable it...
sudo ln -s /etc/nginx/sites-available/atmosphere-vph /etc/nginx/sites-enabled/atmosphere-vph
...
```
sudo cp /home/atmosphere/current/doc/install/support/logrotate/atmosphere-vph /etc/logrotate.d/atmosphere-vph
```
