module OwnedPayloadTests
  ...
  included do
    ...
    describe "POST /owned_payload_path" do
      context 'when unauthenticated' do
        it 'returns 401 authentication error' do
          post api("/#{owned_payload_path}"), params: new_owned_payload
      ...
      context 'when authenticated as user' do
        it 'returns 201 Created on new owned payload created' do
          post api("/#{owned_payload_path}", owner1), params: new_owned_payload
        ...
        it 'creates new owned payload with given owner list' do
          post api("/#{owned_payload_path}", owner1), params: new_owned_payload_with_owners
        ...
        it 'returns 22  Unprocessable Entity when name is missing' do
          post api("/#{owned_payload_path}", owner1), params: {payload_sym => {payload: 'payload'}}
          expect(response.status).to eq 422
        end

        it 'returns 422  Unprocessable Entity when name format is incorect' do
          post api("/#{owned_payload_path}", owner1), params: {payload_sym => {name: 'wrong\name'}}
          expect(response.status).to eq 422
        end

        it 'returns 422 Unprocessable Entity when payload is missing' do
          post api("/#{owned_payload_path}", owner1), params: {payload_sym => {name: 'new/owned_payload'}}
          expect(response.status).to eq 422
        end
      end
    end
    ...
    describe "PUT /owned_payload_path/{id}" do
      ...
      context 'when authenticated as user' do
        it 'returns 200 on success' do
          put api("/#{owned_payload_path}/#{owned_payload1.id}", owner1),
              params: {payload_sym => {name: 'new/owned/payload/name'}}
          expect(response.status).to eq 200
        end
        ...
        it 'updates owned payload payload' do
          ...
          put api("/#{owned_payload_path}/#{owned_payload1.id}", owner1),
              params: {payload_sym => {payload: new_payload}}
        ...
        it 'updates owned payload owners' do
          new_payload = 'new payload'
          put api("/#{owned_payload_path}/#{owned_payload1.id}", owner1),
              params: {payload_sym => {owners: [owner2.id]}}
          updated_owned_payload = owned_payload_class.find(owned_payload1.id)
